...
strip_ansi_xterm(DESC * d, const char *raw)
{
    ...
    //char *q = buf;
    char *r;
    int i_val, i_chk, i_type, i_chk2, i_tagOpen, i_buildingTag;

    bufptr = buf = alloc_lbuf("strip_xterm");

    DPUSH; /* #104 */

    i_val = i_chk = i_type = i_tagOpen = i_buildingTag = 0;
    r = NULL;

    while (p && *p) {
      if(!(*p == ESC_CHAR && (*(p+2) == '3' || *(p+2) == '4'))){
            //We were building an opening tag, so we need to close that part of it.
            if(ShowAnsiMXP(d->player) && i_buildingTag){
              //Write out tag closer and the append ANSI code for MXP
              safe_str(">",buf,&bufptr);
              i_buildingTag = 0;
              i_tagOpen = 1;
            } 
      ...
      if (*p != ESC_CHAR) {
        ...
        if ( (*(p+1) && (*(p+1) == '[')) && (*(p+2) && ((*(p+2) == '4') || (*(p+2) == '3'))) && (*(p+3) && (*(p+3) == '8')) ) {
          ...
          //If the tag is already open, close it so we can reopen a new one
          if(i_tagOpen) {
            safe_str("</C>",buf,&bufptr);
            safe_chr(ESC_CHAR,buf,&bufptr);
            safe_str("[3z",buf,&bufptr);
            i_tagOpen = 0;
          }          

          //If we are MXPColor and we have NOT started building the opening tag...starting building it.
          if(ShowAnsiMXP(d->player) && !i_tagOpen &&!i_buildingTag) {
            //Write out the prepend ANSI code for MXP and start the C tag
            safe_chr(ESC_CHAR,buf,&bufptr);
            safe_str("[1z<C",buf,&bufptr);
            i_buildingTag = 1;
          }
            ...
            if ( i_chk == 2 ) {
              ...
              if ( (i_chk2 >= 0) && (i_chk2 < 256) ) {
                if(!ShowAnsiMXP(d->player)){
                  ...
                  //We're MXPcolors, so lets do that instead of normal ansi
                  r = mxp_colors[i_chk2];

                  
                  if(!i_type) {
                    safe_str(" FORE=#",buf,&bufptr);
                  } else {
                    //if background color, add B=
                    safe_str(" BACK=#",buf,&bufptr);
                  }
                }
                while ( r && *r ) {
                  //*q++ = *r++;
    ...
    //If the tag is still open, close it since we're done parsing
    if(i_tagOpen) {
      safe_str("</c>",buf,&bufptr);
      safe_chr(ESC_CHAR,buf,&bufptr);
      safe_str("[3z",buf,&bufptr);
